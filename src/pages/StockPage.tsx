...
export default function StockPage() {
  ...
  if (!stock) {
    return <div className="max-w-7xl mx-auto px-4 py-10 text-[#F23645]">找不到股票。</div>
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-[#787B86]">{stock.name}</p>
          <div className="flex flex-wrap items-center gap-2">
            <RankBadge rank={stock.sector_rank} total={stock.total_in_sector} label={`${stock.sector_name}排名`} />
            <span className={`text-xs px-1.5 py-0.5 rounded font-mono border ${stock.benchmark === 'QQQ' ? 'bg-[#a78bfa]/10 text-[#a78bfa] border-[#a78bfa]/20' : 'bg-[#2962FF]/10 text-[#2962FF] border-[#2962FF]/20'}`}>
              vs {stock.benchmark}
            </span>
            {stock.industry && (
              <span className="text-[#787B86] text-xs">{stock.industry}</span>
            )}
          </div>
        </div>

        <div className="text-right">
          <div className="text-3xl font-bold text-[#D1D4DC] font-mono">{fmtPrice(stock.price)}</div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {cards.map(c => (
          <div key={c.label} className="bg-[#1E222D] border border-[#2A2E39] px-4 py-3">
            <div className="panel-label mb-1">{c.label}</div>
            <div className={`text-lg font-bold font-mono ${c.extra || 'text-[#D1D4DC]'}`}>{c.value}</div>
          </div>
        ))}
      </div>
