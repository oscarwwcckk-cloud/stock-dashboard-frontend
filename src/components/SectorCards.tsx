...
export default function SectorCards({ sectors }: Props) {
  ...
  return (
    <div className="border border-[#252B3D] divide-y divide-[#252B3D]">
      {/* Column header */}
      <div className="flex items-center px-4 py-1.5 bg-[#131720]">
      ...
      {sorted.map((s, i) => {
        ...
        return (
          <Link key={s.sector_key} to={`/sector/${s.sector_key}`}
                className="flex items-center px-4 py-2.5 hover:bg-[#1C2030] transition-colors group">
            {/* Rank */}
            <span className="w-8 text-[#5C6480] text-xs font-mono shrink-0">
              {i + 1}
            </span>

            {/* Name + RS bar */}
            <div className="flex-1 min-w-0 mr-3">
              <div className="text-[#C8D1E8] text-sm font-medium truncate group-hover:text-white transition-colors">
                {s.sector_name}
              </div>
              <div className="mt-1.5 h-[3px] rounded-full bg-[#252B3D] overflow-hidden">
